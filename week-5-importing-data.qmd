...
#alreday in working directory dont need file path
```
...
#| label: read-nimbis-correct-na
  ...
  #specify what to consider NA
  #need to have used _csv not .csv bc must be in tibble form
```
...
nimbus <- read_csv("nimbus.csv", na = ".",
                   col_types = list(ozone = col_double(),#can add factors to ()
                                    date = col_datetime()))
```
...
```{r}
#| label: load-gsw
gsw <- read_csv("GSWdata.csv", na = "N/A", 
                col_types = list(`2019-20`= col_skip(),
                                 `2020-21`= col_number(),
                                 `FG%` = col_number(),
                                 `3P%`= col_number()))
#very annoying bad varibale names
```
...
write_csv(gsw, file = "gws_clean.csv")
```
